...
async def ocr_api(发票ID: int, 请求: Request, credentials: HTTPAuthorizationCredentials = Depends(security)):
    # 提取token
    ...
    try:
        ...
        数据 = 发票处理实例.发票文件处理(发票ID)
        发票处理实例.发票数据存入数据库(数据, 发票ID)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"发票处理失败: {str(e)}")

    # 返回成功响应到前端
    return {"status": "success", "message": "OCR处理成功", "发票ID": 发票ID}
