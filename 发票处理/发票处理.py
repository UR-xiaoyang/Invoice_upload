...
class 发票处理:
    ...
    def 发票文件处理(发票ID):
        ...
        OCR识别结果 = None
        if ".pdf" in 查询的数据[0][4]:
            OCR识别结果 = OCR_PaddleOCR.ocr_pdf(f"data_upload_invoice/{查询的数据[0][4]}")
        else:
            OCR识别结果 = OCR_PaddleOCR.ocr_image(f"data_upload_invoice/{查询的数据[0][4]}")
        发票结果 = Arrange_invoice.get_invoice_info(OCR识别结果)
